...
func main() {
	var keyword, country, lang, domain string
	var proxy interface{}
	var list bool
	flag.BoolVar(&list, "list", false, "see lists all domain")

	var page, result, timeSleep int
	flag.StringVar(&domain, "d", "hackerone.com", "Results only from this link hackerone.com")
	flag.StringVar(&keyword, "k", "", "keyword search")
	flag.StringVar(&country, "c", "com", "country google if you see if you want see lists use [./hvr -list] or [go run main.go list]")
	flag.StringVar(&lang, "l", "com", "choose language results for google")
	switch v := proxy.(type) {
	case string:
		{
			flag.StringVar(&v, "p", "", "proxy")
		}
	}
	flag.IntVar(&page, "g", 1, "results page in google!")
	flag.IntVar(&result, "r", 10, "The number of results returned from Google")
	flag.IntVar(&timeSleep, "t", 3, "time sleep before go next another page")
	flag.Parse()
	if list {
		for v := range googleDomains {
			fmt.Println(v)
		}
		os.Exit(0)
	}
	dk := fmt.Sprintf("site:%s %s", domain, keyword)
	res, err := GoogleScrape(dk, country, lang, proxy, page, result, timeSleep)
	if err == nil {
		for _, res := range res {
			fmt.Println(res.ResultURL)
		}
	}
}
